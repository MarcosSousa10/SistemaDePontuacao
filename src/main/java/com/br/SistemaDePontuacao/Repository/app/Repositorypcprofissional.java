...
"nvl((select SUM(vltotal) / ?1 AS codbrinde from pcnfsaid where codprofissional = p.codprofissional),0) as codbrinde," +
    ...
    @Query(value = "SELECT a.codprofissional, " + 
        ...
        "where p.cnpj = ?2 and DTSAIDA BETWEEN TO_DATE(?3, 'DD/MM/YYYY') AND TO_DATE(?4, 'DD/MM/YYYY')" +
    ...
    Optional<pcprofissional> informacaofiltrocnpj(Long fatordivisao,String cnpj,String dtsaida,String dtinico);
    @Transactional
