...
import com.br.SistemaDePontuacao.services.ImageService1;

    @RestController
    @CrossOrigin("*")

public class ImageController1 {
    private final ImageService1 imageService;

    public ImageController1(ImageService1 imageService) {
        this.imageService = imageService;
    }
@GetMapping("/images1")
    public ResponseEntity<List<String>> getImageUrls() {
        List<Image1> images = imageService.getAllImages();
        List<String> imageUrls = new ArrayList<>();

        for (Image1 image : images) {
            // Assuming you have a base URL where the images are served from
            String imageUrl = "http://192.168.2.181:5555/images1/" + image.getId();
    ...
    @GetMapping("/images1/{id}")
    public ResponseEntity<byte[]> getImage(@PathVariable Long id) {
        Image1 image = imageService.getImageById(id);
    ...
    public ResponseEntity<String> uploadImage(@RequestBody MultipartFile file) {
        try {
            ...
            Image1 savedImage = imageService.saveImage(imageData);
